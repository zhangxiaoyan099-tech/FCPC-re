# 草稿：\(A_t(M)\) 的定义、跨配对实验与中心优化方案

> **文档状态：研究草稿/诊断记录，不作为论文理论主线。**
> 本文记录了 $A_t(M),U_t(M),D_t(M)$ 的形成过程、冻结检查点实验和“聚合抵消”这一关键负结果。已经确认有效的定义与结论已吸收到 [FCPC_GRAD_THEORY_LEMMAS_4_6.md](FCPC_GRAD_THEORY_LEMMAS_4_6.md)。
> 若本草稿与主线文档冲突，以主线文档为准。特别注意：$A_t$ 下降不蕴含 $U_t$ 必然下降，且这些诊断量不再单独承担 FCPC-grad 的收敛加速证明。

## 草稿保留价值与淘汰结论

- **保留：** $A=U+V$、$0\le U\le A$、残差抵消、归一化 $D_t(M)$、冻结检查点反事实实验和对应实验命令。
- **吸收进主线：** 加权 JS 恒等式、标签分布到梯度偏差的桥梁、聚合只保留标量下降投影的分析。
- **不再使用：** 从 $A_t(M)$ 变小直接宣称全局收敛加快；寻找无条件正常数 $\kappa_{\min}>0$ 使 $U_t\ge\kappa_{\min}A_t$。
- **新定位：** 本文解释“为什么上一版共同中心失败，以及为什么需要 FCPC-grad”，而不是给出最终定理。
