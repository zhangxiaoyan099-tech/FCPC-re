...
> **文档状态：历史方案草稿，不作为当前 FCPC-grad 的最终算法或理论。**
> 本文保留“加权 JS 互补收益—最大权匹配—历史共同参数中心”的早期方案，以及强凸三层分析的探索过程。当前统一理论请引用 [FCPC_GRAD_THEORY_LEMMAS_4_6.md](FCPC_GRAD_THEORY_LEMMAS_4_6.md)。
> 若本草稿与主线冲突，以主线为准。尤其不能再把历史参数平均视为混合分布训练模型，也不能用本文的强凸局部最优点分析替代 ResNet-18 的全局非凸收敛证明。

## 草稿保留价值与后续处理

- **保留：** 加权 KL–JS 恒等式、加权 JS 边权、最大权匹配流程、上一版共同历史中心和二乘二消融设计。
- **吸收进主线：** 配对目标的严格分布解释、最大权匹配、算法边界与 LDP 注意事项。
- **被 FCPC-grad 替代：** 只由两个历史模型加权平均得到的共同中心。当前中心还包含从 $w^t$ 出发的历史更新代理，并使用精确 proximal 与中心裁剪。
- **仅作辅助直觉：** 强凸条件下“本地 SGD—正则化最优点—全局最优点”的三层结构；它不能直接解释当前深度网络的全局加速。
- **需要避免：** 从标签 KL 直接跳到参数中心误差、从局部正则目标收敛直接跳到服务器全局收敛。

> 历史记录日期：2026-09-01
> 对应实现：提交 `1ac731b` 之后的新算法版本  
> 原目的：解释当时的新配对指标、共同历史中心和“三层证明”；这些内容现仅用于追溯方案演变。
