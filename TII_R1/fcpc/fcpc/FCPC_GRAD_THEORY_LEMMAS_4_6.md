...
| $\phi_{p,t}\in[0,1]$ | 代理方向门控；关闭、开启或软门控 |
...
\widetilde c_{p,t}
=w^t+\chi_{p,t}
\big[(1-\xi_t)h_p^t+\xi_ts_t\phi_{p,t}d_p^t\big].
}
\]
...
+\rho_t\chi_{p,t}\xi_ts_t\phi_{p,t}d_p^t.
\end{aligned}
\]
...
### 15.2 FCPC-grad-oracle 方向门控审计

在 seed 42 的冻结检查点 $t\in\{1,10,25,50\}$ 上，五个最大权配对中，oracle 分别保留 $80\%,80\%,60\%,60\%$，说明历史代理中分别存在 $1,1,2,2$ 对方向有害项。未经门控时，服务器显式代理投影在后两个检查点为负：

\[
-\langle g_{25},P_{25}\rangle=-1.869\times10^{-4},
\qquad
-\langle g_{50},P_{50}\rangle=-2.094\times10^{-4}.
\]

oracle 门控后四个检查点均恢复为正，分别为
...
相对干净 `proxy_off` 的实际服务器更新差 $Z_t$，其 $L=0$ 平均一阶收益在四个检查点也均为正，但五条随机轨迹中的正值比例只有 $60\%,60\%,80\%,60\%$。因此该实验支持“有害历史方向是机制缺口，门控可修复引理6的显式投影”，尚不支持“每条本地随机轨迹都获得真实单轮损失改善”。oracle 使用同一个经验 $g_t$ 选方向，所以其一阶投影为正主要是机制校验；下一阶段必须扩大随机轨迹、筛选 $s_t$，并在独立模型种子上验证。
