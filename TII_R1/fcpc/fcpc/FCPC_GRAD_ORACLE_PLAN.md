...
本实验位于主证明之后，回答一个安全增强问题：**当无门控 FCPC-grad 的逐对或服务器整体余量失效时，排除方向错误的配对代理能否改善服务器单轮下降？**

主证明与实验判据的先后顺序是：

1. 无门控逐对平行投影条件；
2. 无门控服务器整体平行投影条件；
3. proximal 轨迹与光滑下降收益；
4. 最后才比较 oracle/可部署门控。
...
历史代理中存在方向错误项，但门控不再作为主证明入口。主线先检验逐对有害平行投影条件和服务器整体平行投影条件；只有这些条件失效时，才研究只使用历史服务器更新的可部署门控。

### 6.2 两者都好

原引理 4 的全范数充分界过于保守；0% 的成立率不代表真实代理机制无效。理论主线改为逐对平行投影余量和服务器整体余量，oracle 门控仅用于测量可获得的安全上限。
...
5. 单轮审计不能替代多轮收敛实验；只有无门控主条件持续失效且 oracle 明显改善时，才有必要设计可部署历史门控并单独训练。
